	...
	const handlePlanificar = async () => {
		...
		try {
			...
			if (error.response) {
				...
				// 1. Obtenemos el mensaje crudo que envía el backend
				let rawMessage =
					data.detail ||
					data.message ||
					data.error ||
				...
				// 2. Verificamos si el mensaje contiene el error de restricción única
				if (
					typeof rawMessage === "string" &&
					rawMessage.includes("duplicate key value violates unique constraint")
				) {
					mensajeError =
						"Error al planificar, las cantidades de las lineas disminuyeron, debe replanificar un dia antes a la fecha ingresada.";
				} else {
					// Si no coincide, mostramos el error original
					mensajeError = rawMessage;
				}
	...
	const handleReplanificar = async () => {
		...
		try {
			...
			const response = await api.post(
				"/planificacion/replanificar-ops-por-capacidad/",
				payload
			);
