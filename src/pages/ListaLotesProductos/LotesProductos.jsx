...
const LotesProductos = ({ products }) => {
    ...
    try {
      ...
      const contenido = `
        <div style="text-align: center; margin-bottom: 20px;">
          ...
          <h3 style="margin: 0 0 15px 0; color: #3498db; font-size: 16px; font-weight: 600;">${lote.producto_nombre}</h3>
        </div>
        ...
        <div style="text-align: center; font-size: 12px; color: #666; line-height: 1.4;">
          <p style="margin: 5px 0;"><strong>Lote #${lote.id_lote_produccion}</strong></p>
          <p style="margin: 5px 0;">Producción: ${formatDate(lote.fecha_produccion)}</p>
          <p style="margin: 5px 0;">Vencimiento: ${formatDate(lote.fecha_vencimiento)}</p>
      ...
      pdf.save(`QR_Lote_${lote.id_lote_produccion}_${lote.producto_nombre.replace(/\s+/g, '_')}.pdf`);
  ...
  const generarQRPDFAlternativo = async (lote) => {
    ...
    pdf.setFontSize(12);
    pdf.text(`Lote #${lote.id_lote_produccion}`, pdfWidth / 2, 155, { align: 'center' });
    pdf.text(`Producción: ${formatDate(lote.fecha_produccion)}`, pdfWidth / 2, 165, { align: 'center' });
    pdf.text(`Vencimiento: ${formatDate(lote.fecha_vencimiento)}`, pdfWidth / 2, 175, { align: 'center' });
    pdf.text('Escanea para ver la trazabilidad', pdfWidth / 2, 185, { align: 'center' });
              ...
              <div key={lote.id_lote_produccion} className={styles.loteCard}>
                ...
                <div className={styles.loteDetails}>
                  <div className={styles.loteDetail}>
                    ...
                    <span className={styles.detailValue}>{lote.producto_nombre}</span>
                  </div>
                ...
                {obtenerNumerosPaginaLotes().map(numero => (
                  <button
                    ...
                    className={`${styles.lotesNumeroPagina} ${
                      paginaActualLotes === numero ? styles.lotesPaginaActiva : ''
                    }`}
                  >
                    {numero}
                  </button>
